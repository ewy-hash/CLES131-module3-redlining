...
crs(Baltimore)
crs(NDVIbaltimore)
#however according to this they are different EPSG (i found this command a lot later and came back retroactively)
...
#for this, the error bars represent the 95% quantile, not error calculations
...
#low r ^2 but low p value indicates the model it came up with isnt good, but there is a strong correlation between the variables. can maybe be interpreted as grade is 9% of the cause of change in ndvi.
...
```{r}
#idk why group works but it does: answer : group 1 means it treats all data as one group
combined_cor_plot <- ggplot(data = combined, mapping = aes(x = grade, y = NDVI_mean$T12SUC_20211110T180601_B08))+
  ...
  labs(title = "NDVI compared to grade of US Cities")


#it does seem like NDVI is associated with historical redlining. lets check to see if it differs by city
...
plot_grid(combined_cor_plot, baltimore_cor_plot, Indianapolis_cor_plot, phoenix_cor_plot, Minneapolis_cor_plot, Birmingham_cor_plot)

#this plot grid using cowplot shows all of them and they all have a downwards trend and even with geom smooth there is still a clear correlation
```
### Bonus 2 (1 point)
