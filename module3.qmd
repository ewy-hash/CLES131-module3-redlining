...
```{r}
# seems like they do share the coordiante system, 
#as you can see from running the code below they 
#both get placed onto the map using longitude and latitude coords

crs(Baltimore)
crs(NDVIbaltimore)
#however according to this they are different EPSG
#(i found this command a lot later and came back retroactively)

test <- ggplot(data = Baltimore)+
  geom_sf()+
  geom_spatraster(data = NIRbaltimore, alpha = 0.5)
test
#so im confused about why this worked then
```
### Q5 (1 points)
...
# also the raster data is showing everywhere 
#when it should be showing only in the polygons.
#need to mask, to use mask( ) both need to be spatwhatever
...
#sooooo
#now made this color table to draw from as opposed to getting from baltimore
...
#google says use ggnewscale to have multiple scales 
#so i can have inside and inverse raster data
#install.packages("ggnewscale")
library(ggnewscale)
...
```{r}
#reconfigured to work for each of the cities 
#by just changing the file names and a few other things
...
NDVIplot<- terra::project(x = NDVIphoenix, "EPSG:4326")
...
```{r}
vectbaltimore <- terra::vect(Baltimore)
#step 1 made polygons with just the grade data, then conv to vect
#this was from office hours but it isn't working and I don't understand why

# temp <- Baltimore |> 
#   mutate(ndvi_mean = terra::extract(NDVIbaltimore, vectbaltimore,
#                                     fun = mean, na.rm = TRUE, ID = FALSE) |> 
#            pull()
#          )

#quantile(x = temp$T18SUJ_20230525T154941_B08, probs= .95)
...
ndvi_values <- terra::extract(NDVIplot, vect(Baltimore),
                                    fun = mean, na.rm = TRUE, ID = FALSE)

Baltimore$NDVI_mean <- ndvi_values
...
NDVIplot<- terra::project(x = NDVIphoenix, "EPSG:4326")
ndvi_values <- terra::extract(NDVIplot, vect(Phoenix),
                                    fun = mean, na.rm = TRUE, ID = FALSE)

Phoenix$NDVI_mean <- ndvi_values
...
NDVIplot<- terra::project(x = NDVI, "EPSG:4326")
ndvi_values <- terra::extract(NDVIplot, vect(Minneapolis),
                                    fun = mean, na.rm = TRUE, ID = FALSE)

Minneapolis$NDVI_mean <- ndvi_values
...
ndvi_values <- terra::extract(NDVIplot, vect(Birmingham),
                                    fun = mean, na.rm = TRUE, ID = FALSE)

Birmingham$NDVI_mean <- ndvi_values
...
NDVIplot<- terra::project(x = NDVI, "EPSG:4326")
ndvi_values <- terra::extract(NDVIplot, vect(Indianapolis),
                                    fun = mean, na.rm = TRUE, ID = FALSE)

Indianapolis$NDVI_mean <- ndvi_values

#quantile and calculations in a df
mean = c(mean(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, na.rm = TRUE),
          mean(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, na.rm = TRUE),
          mean(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, na.rm = TRUE),
          mean(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, na.rm = TRUE),
          mean(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08))

median = c(median(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, na.rm = TRUE),
           median(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, na.rm = TRUE),
           median(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, na.rm = TRUE),
           median(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, na.rm = TRUE),
           median(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08))

mean(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, na.rm = TRUE)
median(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, na.rm = TRUE)
quantile(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, probs = c(.025, .975))

mean(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, na.rm = TRUE)
median(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, na.rm = TRUE)
quantile(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, probs = c(.025, .975))

mean(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, na.rm = TRUE)
median(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, na.rm = TRUE)
quantile(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, probs = c(.025, .975))

mean(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, na.rm = TRUE)
median(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, na.rm = TRUE)
quantile(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, probs = c(.025, .975))

#Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08 |> 
  mean(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08)
median(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08)
quantile(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08, probs = c(.025, .975))


summary <- data.frame(cities = c("Indianapolis", "Birmingham", "Phoenix", "Baltimore", "Minneapolis"), 
                      mean = c(mean(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, na.rm = TRUE),
                               mean(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, na.rm = TRUE),
                               mean(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, na.rm = TRUE),
                               mean(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, na.rm = TRUE),
                      ...
                      median = c(median(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, na.rm = TRUE),
                                 ...
                                 median(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, na.rm = TRUE),
                                 median(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08)),
                      confidenceMin = c(quantile(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, probs = .025),
                               quantile(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, probs = .025),
                               quantile(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, probs = .025),
                               quantile(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, probs = .025),
                               quantile(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08, probs = .025)),
                      confidenceMax = c(quantile(Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08, probs = .975),
                               quantile(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08, probs = .975),
                               quantile(Phoenix$NDVI_mean$T12SUC_20211110T180601_B08, probs = .975),
                               quantile(Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08, probs = .975),
                               quantile(Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08, probs = .975)))
                               


#code not being used but i didn't want to delete
# gradeA <- filter(Baltimore, grade == "A")
# gradeB <- filter(Baltimore, grade == "B")
# gradeC <- filter(Baltimore, grade == "C")
# gradeD <- filter(Baltimore, grade == "D")
# 
# #vects
# gradeAvect <- vect(gradeA)
# gradeBvect <- vect(gradeB)
# gradeCvect <- vect(gradeC)
# gradeDvect <- vect(gradeD)
# 
# 
# #extracted
# extA <- extract(x = NDVIbaltimore, y = gradeAvect, method = "simple" )
# extB <- extract(x = NDVIbaltimore, y = gradeBvect, method = "simple" )
# extC <- extract(x = NDVIbaltimore, y = gradeCvect, method = "simple" )
# extD <- extract(x = NDVIbaltimore, y = gradeDvect, method = "simple" )
# 
# 
# mean(extA$T18SUJ_20230525T154941_B08)
# median(extA$T18SUJ_20230525T154941_B08)
# quantile(x = extA$T18SUJ_20230525T154941_B08, probs = c(.025, 0.975))
# 
# mean(extB$T18SUJ_20230525T154941_B08)
# median(extB$T18SUJ_20230525T154941_B08)
# quantile(extB$T18SUJ_20230525T154941_B08, probs = c(.025, 0.975))
# 
# mean(extC$T18SUJ_20230525T154941_B08)
# median(extC$T18SUJ_20230525T154941_B08)
# quantile(extC$T18SUJ_20230525T154941_B08, probs = c(.025, 0.975))
# 
# 
# mean(extD$T18SUJ_20230525T154941_B08)
# median(extD$T18SUJ_20230525T154941_B08)
# quantile(extD$T18SUJ_20230525T154941_B08, probs = c(.025, 0.975))
# 
# lm(formula = data = extA$T18SUJ_20230525T154941_B08)

ggplot(summary, mapping = aes(x = cities, y = mean))+
  geom_point()+
  geom_errorbar(aes(ymin = confidenceMin, ymax = confidenceMax))
#for this, the error bars represent the 95% quantile, 
#not error calculations
#why do i need the other thing after ndvi_mean



ggplot(data = Birmingham, mapping = aes(x = grade, y = NDVI_mean$T16SEC_20220427T162829_B08))+
  geom_bar(stat = "summary", fun = "mean")
#seems like compared to the other cities, birmingham
#has higher NDVI for D grade. None of the other cities 
#have higher NDVI for D grade.


combined <- rbind(Phoenix, Baltimore, Birmingham, Minneapolis, Indianapolis)


ggplot(data = combined, mapping = aes(x = grade, y = NDVI_mean$T12SUC_20211110T180601_B08))+
  geom_bar(stat = "summary", fun = "mean")

#mean(combined$NDVI_mean$T12SUC_20211110T180601_B08[combined$grade == "A"])
```
### Bonus 1 (1 point)
Perform a statistical test to support your visual analysis above. 
```{r}
linear_model <- lm(combined$NDVI_mean$T12SUC_20211110T180601_B08 ~ combined$grade, data = combined)
summary(linear_model)


summary(lm(Birmingham$NDVI_mean$T16SEC_20220427T162829_B08 ~ Birmingham$grade, data = Birmingham))
#low r ^2 but low p value indicates the model it came 
#up with isnt good, but there is a strong correlation 
#between the variables. can maybe be interpreted as 
#grade is 9% of the cause of change in ndvi.
summary(anova(linear_model))
```
...
```{r}
#idk why group works but it does: answer : group 1 means 
#it treats all data as one group
combined_cor_plot <- ggplot(data = combined, mapping = aes(x = grade, y = NDVI_mean$T12SUC_20211110T180601_B08))+
  geom_smooth(aes(group = 1))+
  scale_y_continuous(name = "NDVI")+
  labs(title = "NDVI compared to grade of 5 US Cities")


#it does seem like NDVI is associated with historical 
#redlining. lets check to see if it differs by city

#birmingham
Birmingham_cor_plot<- ggplot(data = Birmingham, mapping = aes(x = grade, y = Birmingham$NDVI_mean$T16SEC_20220427T162829_B08))+
  geom_smooth(aes(group = 1))+
  scale_y_continuous(name = "NDVI")+
  labs(title = "NDVI compared to grade in Birmingham")

#phoenix
phoenix_cor_plot <-ggplot(data = Phoenix, mapping = aes(x = grade, y = Phoenix$NDVI_mean$T12SUC_20211110T180601_B08))+
  geom_smooth(aes(group = 1))+
  scale_y_continuous(name = "NDVI")+
  labs(title = "NDVI compared to grade in Phoenix")

#indianapolis
Indianapolis_cor_plot <-ggplot(data = Indianapolis, mapping = aes(x = grade, y = Indianapolis$NDVI_mean$T16TEK_20210726T162901_B08))+
  geom_smooth(aes(group = 1))+
  scale_y_continuous(name = "NDVI")+
  labs(title = "NDVI compared to grade in Indianapolis")

#minneapolis
Minneapolis_cor_plot <-ggplot(data = Minneapolis, mapping = aes(x = grade, y = Minneapolis$NDVI_mean$T15TVK_20220603T170901_B08))+
  geom_smooth(aes(group = 1))+
  scale_y_continuous(name = "NDVI")+
  labs(title = "NDVI compared to grade in Minneapolis")

#baltimore
baltimore_cor_plot <-ggplot(data = Baltimore, mapping = aes(x = grade, y = Baltimore$NDVI_mean$T18SUJ_20230525T154941_B08))+
  geom_smooth(aes(group = 1))+
  scale_y_continuous(name = "NDVI")+
  labs(title = "NDVI compared to grade in Baltimore")

plot_grid(combined_cor_plot, baltimore_cor_plot, Indianapolis_cor_plot, phoenix_cor_plot, Minneapolis_cor_plot, Birmingham_cor_plot, ncol = 2)
ggsave(filename = "CombinedPLOT.png", 
       path = "PLOTS/", 
       width = 8, 
       height = 6, 
       dpi = 600)

#this plot grid using cowplot shows all of them and they
#all have a downwards trend and even with geom smooth there
#is still a clear correlation, except birmingham where d is 
#higher than c but still lower than a

#I chose to do a line instead of a bar just because I thought it was
...
All cities except for birmingham have a p value less that .05. This means that for all besides Birmingham, there is likely a correlation between grade and NDVI. However when all the data is combined, then the p value is less then .05, indicating a correlation between grade and NDVI for these cities. The confidence interval should be shown by the geom smooth function on my final graph.
